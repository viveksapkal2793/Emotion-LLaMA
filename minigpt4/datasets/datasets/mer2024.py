...
class MER2024Dataset(Dataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_path):
        ...
        self.feat_path = "/scratch/data/bikash_rs/vivek/dataset/MER2024_feats"
        # self.tmp = [x.strip().split(' ') for x in open(ann_path)]
        # print(('video number:%d' % (len(self.tmp))))

        # Read CSV file instead of text file
        self.mer_data = pd.read_csv(ann_path)
        print(f'MER2024 video number: {len(self.mer_data)}')

        self.emos = ['neutral', 'angry', 'happy', 'sad', 'worried', 'surprise']
        ...
        self.character_lines = pd.read_csv('/scratch/data/bikash_rs/vivek/dataset/MER2024_feats/transcription_all_new.csv')

    def __len__(self):
        # return len(self.tmp)
        return len(self.mer_data)
    ...
    def __getitem__(self, index):
        # Get data from CSV
        row = self.mer_data.iloc[index]
        video_name = row['name']
        emotion_label = row['discrete'].lower()  # Get emotion from 'discrete' column

        # Load only the three feature modalities (no image/EVA needed)
        FaceMAE_feats, VideoMAE_feats, Audio_feats = self.get(video_name)
        ...
        # Task and instruction setup
        task = random.choice(self.task_pool)
        if task == "emotion":
            caption = emotion_label  # Use emotion from CSV
            caption = self.text_processor(caption)
            instruction_pool = self.emotion_instruction_pool
        ...
        # Get transcription
        sentence_match = self.character_lines.loc[self.character_lines['name'] == video_name, 'sentence_en']
        if len(sentence_match) > 0:
            sentence = sentence_match.values[0]
        else:
            sentence = ""  # Default empty if not found

        character_line = f"The person in video says: {sentence}. " if sentence else ""
        
        # Remove <video><VideoHere></video> since we're not using images
        instruction = "<feature><FeatureHere></feature> {} [{}] {} ".format(
    ...
    def get(self, video_name):
        # FaceMAE feature
        FaceMAE_feats_path = os.path.join(self.file_path, 'mae_340_24_UTT_1', video_name + '.npy') # MER2024
        FaceMAE_feats = torch.tensor(np.load(FaceMAE_feats_path))

        # VideoMAE feature
        VideoMAE_feats_path = os.path.join(self.file_path, 'maeVideo_399_24_UTT_1', video_name + '.npy')
        VideoMAE_feats = torch.tensor(np.load(VideoMAE_feats_path))

        # Audio feature
        Audio_feats_path = os.path.join(self.file_path, 'HL_24_UTT_1', video_name + '.npy')
